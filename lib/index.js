...
class MikudosSocketIoClient {
    constructor({ uri, option = {} }, { rootNamespace = '', rpcEventName = 'rpc-call', chatEventName = 'message', duplexEventName = 'stream-call', } = {}, saveTokenCallback = (token) => { }, getTokenMethod = () => this.jwt) {
        ...
        this.duplexEventEmitter = new events_1.EventEmitter();
        if (!uri)
        ...
        this.socket = socket_io_client_1.default(`${uri}${rootNamespace}`, option);
        this.init();
    }
    init() {
    ...
    async duplexCall(method, data, room = '') {
        return await new Promise((resolve, reject) => {
            this.socket.emit(this.duplexEventName, {
                method,
                room,
                data,
            }, (data) => {
                ...
                this.socket.on(`${this.duplexEventName} ${method}`, (md) => this.duplexEventEmitter.emit(method, md));
                resolve(data);
            });
        });
    }
    async duplexSend(method, data) {
        return await new Promise((resolve, reject) => {
            this.socket.emit(`${this.duplexEventName} send`, { method, data }, (data) => {
                if (data.error)
                    reject(data);
                resolve(data);
            });
    ...
    async duplexCancelCall(method, data = null) {
        return await new Promise((resolve, reject) => {
            this.socket.emit(`${this.duplexEventName} cancel`, { method, data }, (data) => {
                if (data.error)
                    reject(data);
                resolve(data);
            });
